...
class DeepGalaxyTraining(object):
    ...
    def load_data(self, data_fn, test_size=0.3, random=True):
        ...
        print("shapes:", self.x_train.shape, self.x_test.shape, self.y_train.shape, self.y_test.shape)
        ...
        if self.distributed_training is True:
            if hvd.rank() == 0:
                if self.use_noise is True:
                    self.model.save('model_hvd_bw_%d_B0_with_noise_n_p_%d.h5' % (self.input_shape[0], hvd.size()))
                else:
                    self.model.save('model_hvd_bw_%d_B0_no_noise_%d_nodes.h5' % (self.input_shape[0], hvd.size()))
        else:
            if self.use_noise is True:
                self.model.save('model_bw_%d_B0_with_noise.h5' % (self.input_shape[0]))
            else:
                self.model.save('model_bw_%d_B0_no_noise.h5' % (self.input_shape[0]))

    def validate(self):
        y_pred = self.model.predict(self.x_test)
        print(precision_recall_fscore_support(self.y_test, np.argmax(y_pred, axis=1)))
...
if __name__ == "__main__":
    ...
    dgtrain.use_noise = False 
    dgtrain.initialize()
    print('Loading data...')
    dgtrain.load_data('training.h5')
    # dgtrain.input_shape = (224,224,3)
