...
class CovidNet(nn.Module):
    def __init__(self, opts, model='small'):
        ...
        first_linear_shape = self(torch.randn((1, 3, opts.img_size, opts.img_size)), True).shape[-1]
        self.add_module('fc1', nn.Linear(first_linear_shape, 1024))

        self.add_module('fc2', nn.Linear(1024, 256))
        self.add_module('classifier', nn.Linear(256, opts.n_classes))

    def forward(self, x, get_shape=False):
        return self.__forward__(x, get_shape)

    def forward_large_net(self, x, get_shape=False):
    ...
    def forward_small_net(self, x, get_shape=False):
        x = F.max_pool2d(F.relu(self.conv1(x)), 2)
